...
#ax.set_xticklabels(fontsize=14)
    ...
    if choice1=='Pressure':
        for k in range(num_fgs):
           ax.plot(df_final_list[k]['TVDSS'],df_final_list[k]['PRESSURE'],marker="v",lw=2.5,label=label_wekk_p[k],markersize=12)
...
#ax2.invert_yaxis()
    ...
    ax.legend(loc='upper right', fontsize='x-large',bbox_to_anchor=(0.76, 0.99))
    ax2.legend(loc='upper right', fontsize='x-large',bbox_to_anchor=(0.99, 0.99))
    ax2.tick_params( axis='y',labelsize=14,direction='out', length=6, width=2, colors='black',
               grid_color='r', grid_alpha=0.5)
    ax2.set_xlabel(xlabel="Temperature ",color="blue",fontsize=18)
